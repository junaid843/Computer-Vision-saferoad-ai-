...
# ─── Line Crossing Module (new feature) ──────────────────────────────────────
...
def mock_detect(image_np):
    ...
    Replace with actual model inference from models/detector.py.
    """
    ...
    for i in range(n):
        ...
        y1 = random.randint(30, max(31, h // 2))
...
if "lc_results" not in st.session_state:
    st.session_state.lc_results = {}        # track_id → belt/plate result dict
...
with st.sidebar:
    ...
    <div style='margin-bottom:24px'>
        ...
        <div style='font-size:0.7rem;color:#5a6075;letter-spacing:0.12em;text-transform:uppercase;font-family:"Space Mono",monospace'>Violation Monitor v1.1</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── Virtual Line Settings (used by Tab 4) ─────────────────────────────────
            ...
            if st.button("▶  Process Video", use_container_width=True):
                ...
                while cap.isOpened():
                    ...
                    if frame_num % sample_every == 0:
                        ...
                        preview_slot.image(annotated, caption=f"Frame {frame_num}",
                                           use_container_width=True)
                    ...
                    status_text.caption(
                        f"Frame {frame_num}/{total_frames} · {processed} sampled · {video_viols} violations"
                    )
                    frame_num += 1
...
with tab4:
    ...
    <div style='background:#12151e;border:1px solid #252a3a;border-radius:12px;
        ...
        <div style='font-size:0.75rem;color:#5a6075;font-family:"Space Mono",monospace;line-height:1.8'>
            <span style='color:#f5c518;font-weight:700'>How it works:</span>
            Define a virtual detection line · Upload video · Any vehicle that
            <em>crosses</em> the line is automatically inspected for seatbelt compliance ·
            Violations are logged with plate number, confidence, and frame reference.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with lc_col1:
        ...
        if lc_video_file:
            # Write to temp file once for reuse
            with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp_lc:
                tmp_lc.write(lc_video_file.read())
                lc_tmp_path = tmp_lc.name

            # Grab first frame for line-placement preview
            cap_prev = cv2.VideoCapture(lc_tmp_path)
            ret_p, first_frame = cap_prev.read()
            cap_prev.release()
            ...
            if ret_p:
                ...
                st.image(
                    preview_with_line,
                    caption=(
                        f"Line preview — {lc_axis} at "
                        f"{'Y' if lc_axis == 'horizontal' else 'X'}={lc_line_pos}"
                    ),
                    use_container_width=True
                )
                ...
                lc_sample_every = st.number_input(
                    "Process every N-th frame", 1, 30, 3, key="lc_sample"
                )
                lc_iou_thresh = st.slider(
                    "Tracker IoU threshold", 0.15, 0.70, 0.30, 0.05, key="lc_iou"
                )
                lc_max_missed = st.number_input(
                    "Max missed frames before track drop", 2, 30, 8, key="lc_missed"
                )
                ...
                if run_lc:
                    ...
                    while cap_lc.isOpened():
                        ...
                        if frame_num_lc % lc_sample_every == 0:
                            ...
                            # Replace with real YOLO vehicle detector:
                            # bboxes = vehicle_yolo.predict(rgb_lc, classes=[2,3,5,7], conf=0.5)
                            # vehicle_bboxes = [tuple(map(int,b)) for b in bboxes[0].boxes.xyxy.tolist()]
                            ...
                            # ── Step 3: Inspect each crossing vehicle ─────────
                            for ev in events:
                                crossing_count += 1

                                # Seatbelt + plate detection on the cropped vehicle region
                                # Replace with: crop_dets = seatbelt_detector.detect(ev.frame_crop)
                            ...
                            # ── Step 4: Annotate and stream frame ─────────────
    ...
    # ── Right column: live preview placeholder + events log ──────────────────
    with lc_col2:
        st.markdown('<div class="section-title">Live Preview</div>', unsafe_allow_html=True)
        ...
        # Crossing events log (shown after analysis)
        if st.session_state.lc_events_log:
            ...
            for ev in st.session_state.lc_events_log[:15]:
                ...
                border_col = "#ff3b3b" if is_viol else "#00e5a0"
                badge_html = (
                    "<span class='violation-badge'>⚠ VIOLATION</span>"
                    if is_viol else
                    "<span class='safe-badge'>✓ COMPLIANT</span>"
                ...
                plate_html = (
                    f"Plate: <span style='color:#f5c518;font-weight:700;letter-spacing:0.1em'>"
                    f"{ev['plate']}</span> &nbsp;|&nbsp; "
                    if is_viol else ""
                )
                st.markdown(f"""
                <div class="log-entry" style='border-left-color:{border_col}'>
                    <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:6px'>
                        {badge_html}
                        <span style='color:#5a6075;font-size:0.72rem'>
                            ID:{ev.get("track_id", "?")} · {ev['timestamp']}
                        </span>
                    </div>
